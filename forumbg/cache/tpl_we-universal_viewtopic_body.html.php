...
<div id="message">
	<ul class="forums-wrapper">
		<li class="author-column"><?php echo ((isset($this->_rootref['L_AUTHOR'])) ? $this->_rootref['L_AUTHOR'] : ((isset($user->lang['AUTHOR'])) ? $user->lang['AUTHOR'] : '{ AUTHOR }')); ?></li>
		<li class="message-column"><?php echo ((isset($this->_rootref['L_MESSAGE'])) ? $this->_rootref['L_MESSAGE'] : ((isset($user->lang['MESSAGE'])) ? $user->lang['MESSAGE'] : '{ MESSAGE }')); ?></li>
	</ul>
	<?php $_postrow_count = (isset($this->_tpldata['postrow'])) ? sizeof($this->_tpldata['postrow']) : 0;if ($_postrow_count) {for ($_postrow_i = 0; $_postrow_i < $_postrow_count; ++$_postrow_i){$_postrow_val = &$this->_tpldata['postrow'][$_postrow_i]; ?>
		...
		<div id="p<?php echo $_postrow_val['POST_ID']; ?>" class="post-wrapper <?php if (($_postrow_val['S_ROW_COUNT'] & 1)  ) {  ?>bg1<?php } else { ?>bg2<?php } if ($_postrow_val['S_UNREAD_POST']) {  ?> unreadpost<?php } if ($_postrow_val['S_POST_REPORTED']) {  ?> reported<?php } ?>">
			<?php if ($_postrow_val['S_IGNORE_POST']) {  ?>

				<p class="error" style="margin: 10px;"><?php echo $_postrow_val['L_IGNORE_POST']; ?></p>
			<?php } else { ?>

				<div <?php if ($_postrow_val['S_FIRST_UNREAD']) {  ?>id="unread"<?php } ?> class="post">
					<div id="profile<?php echo $_postrow_val['POST_ID']; ?>" class="author-column postprofile">
						<div class="post-header">
							<?php if ($_postrow_val['ONLINE_IMG']) {  ?>

								<div class="online"><?php echo $_postrow_val['ONLINE_IMG']; ?></div>
							<?php } ?>
						...
						<div class="profile-wrapper">
							<?php if ($_postrow_val['POSTER_AVATAR']) {  ?>
								...
								<div class="avatar">
									<?php if ($_postrow_val['U_POST_AUTHOR']) {  ?><a href="<?php echo $_postrow_val['U_POST_AUTHOR']; ?>"><?php echo $_postrow_val['POSTER_AVATAR']; ?></a><?php } else { echo $_postrow_val['POSTER_AVATAR']; } ?>

								</div>
							<?php } ?>

							
							<p><?php if (! $_postrow_val['U_POST_AUTHOR']) {  ?><strong><?php echo $_postrow_val['POST_AUTHOR_FULL']; ?></strong><?php } else { echo $_postrow_val['POST_AUTHOR_FULL']; } ?></p>
							<?php if ($_postrow_val['RANK_TITLE']) {  ?><p><?php echo $_postrow_val['RANK_TITLE']; ?></p><?php } if ($_postrow_val['RANK_IMG']) {  ?><p><?php echo $_postrow_val['RANK_IMG']; ?></p><?php } if (! $this->_rootref['S_IS_BOT'] && ( $_postrow_val['U_PM'] || $_postrow_val['U_EMAIL'] || $_postrow_val['U_WWW'] || $_postrow_val['U_MSN'] || $_postrow_val['U_ICQ'] || $_postrow_val['U_YIM'] || $_postrow_val['U_AIM'] || $_postrow_val['U_JABBER'] )) {  ?>
							...
							<?php } if ($_postrow_val['POSTER_POSTS'] != ('') || $_postrow_val['POSTER_JOINED'] || $_postrow_val['POSTER_FROM'] || $_postrow_val['POSTER_WARNINGS']) {  ?>

								<ul class="profile-details">
									<?php if ($_postrow_val['POSTER_POSTS'] != ('')) {  ?><li><strong><?php echo ((isset($this->_rootref['L_POSTS'])) ? $this->_rootref['L_POSTS'] : ((isset($user->lang['POSTS'])) ? $user->lang['POSTS'] : '{ POSTS }')); ?>:</strong> <?php echo $_postrow_val['POSTER_POSTS']; ?></li><?php } if ($_postrow_val['POSTER_JOINED']) {  ?><li><strong><?php echo ((isset($this->_rootref['L_JOINED'])) ? $this->_rootref['L_JOINED'] : ((isset($user->lang['JOINED'])) ? $user->lang['JOINED'] : '{ JOINED }')); ?>:</strong> <?php echo $_postrow_val['POSTER_JOINED']; ?></li><?php } if ($_postrow_val['POSTER_FROM']) {  ?><li><strong><?php echo ((isset($this->_rootref['L_LOCATION'])) ? $this->_rootref['L_LOCATION'] : ((isset($user->lang['LOCATION'])) ? $user->lang['LOCATION'] : '{ LOCATION }')); ?>:</strong> <?php echo $_postrow_val['POSTER_FROM']; ?></li><?php } if ($_postrow_val['POSTER_WARNINGS']) {  ?><li><strong><?php echo ((isset($this->_rootref['L_WARNINGS'])) ? $this->_rootref['L_WARNINGS'] : ((isset($user->lang['WARNINGS'])) ? $user->lang['WARNINGS'] : '{ WARNINGS }')); ?>:</strong> <?php echo $_postrow_val['POSTER_WARNINGS']; ?></li><?php } $_custom_fields_count = (isset($_postrow_val['custom_fields'])) ? sizeof($_postrow_val['custom_fields']) : 0;if ($_custom_fields_count) {for ($_custom_fields_i = 0; $_custom_fields_i < $_custom_fields_count; ++$_custom_fields_i){$_custom_fields_val = &$_postrow_val['custom_fields'][$_custom_fields_i]; ?>

										<li><strong><?php echo $_custom_fields_val['PROFILE_FIELD_NAME']; ?>:</strong> <?php echo $_custom_fields_val['PROFILE_FIELD_VALUE']; ?></li>
									<?php }} ?>
					...
					<div class="message-column postbody">
						<div class="post-header">
							<h4 class="subject <?php if ($_postrow_val['S_FIRST_ROW']) {  ?>first<?php } ?>"><?php if ($_postrow_val['POST_ICON_IMG']) {  ?><img src="<?php echo (isset($this->_rootref['T_ICONS_PATH'])) ? $this->_rootref['T_ICONS_PATH'] : ''; echo $_postrow_val['POST_ICON_IMG']; ?>" width="<?php echo $_postrow_val['POST_ICON_IMG_WIDTH']; ?>" height="<?php echo $_postrow_val['POST_ICON_IMG_HEIGHT']; ?>" alt="" /> <?php } ?> <a href="#p<?php echo $_postrow_val['POST_ID']; ?>"><?php echo $_postrow_val['POST_SUBJECT']; ?></a></h4>
							
							<?php if (! $this->_rootref['S_IS_BOT'] && ( $_postrow_val['U_EDIT'] || $_postrow_val['U_DELETE'] || $_postrow_val['U_REPORT'] || $_postrow_val['U_WARN'] || $_postrow_val['U_INFO'] || $_postrow_val['U_QUOTE'] )) {  ?>
						...
						<div class="content-wrapper">
							<p class="author"><?php if ($this->_rootref['S_IS_BOT']) {  echo $_postrow_val['MINI_POST_IMG']; } else { ?><a href="<?php echo $_postrow_val['U_MINI_POST']; ?>"><?php echo $_postrow_val['MINI_POST_IMG']; ?></a><?php } echo $_postrow_val['POST_DATE']; ?></p>
								
							<?php if ($_postrow_val['S_POST_UNAPPROVED'] || $_postrow_val['S_POST_REPORTED']) {  ?>

								<p class="rules">
									<?php if ($_postrow_val['S_POST_UNAPPROVED']) {  echo (isset($this->_rootref['UNAPPROVED_IMG'])) ? $this->_rootref['UNAPPROVED_IMG'] : ''; ?> <strong><a href="<?php echo $_postrow_val['U_MCP_APPROVE']; ?>"><?php echo ((isset($this->_rootref['L_POST_UNAPPROVED'])) ? $this->_rootref['L_POST_UNAPPROVED'] : ((isset($user->lang['POST_UNAPPROVED'])) ? $user->lang['POST_UNAPPROVED'] : '{ POST_UNAPPROVED }')); ?></a></strong><br /><?php } if ($_postrow_val['S_POST_REPORTED']) {  echo (isset($this->_rootref['REPORTED_IMG'])) ? $this->_rootref['REPORTED_IMG'] : ''; ?> <strong><a href="<?php echo $_postrow_val['U_MCP_REPORT']; ?>"><?php echo ((isset($this->_rootref['L_POST_REPORTED'])) ? $this->_rootref['L_POST_REPORTED'] : ((isset($user->lang['POST_REPORTED'])) ? $user->lang['POST_REPORTED'] : '{ POST_REPORTED }')); ?></a></strong><?php } ?>
				...
				<div class="post-footer">
					<?php if ($_postrow_val['S_PROFILE_FIELD1']) {  ?>

						<ul class="profile-details">
							<!-- Use a construct like this to include admin defined profile fields. Replace FIELD1 with the name of your field. -->
							<li><strong><?php echo $_postrow_val['PROFILE_FIELD1_NAME']; ?>:</strong> <?php echo $_postrow_val['PROFILE_FIELD1_VALUE']; ?></li>
						</ul>
					<?php } ?>

					
					<div class="back2top"><a href="#wrapper" class="top" title="<?php echo ((isset($this->_rootref['L_BACK_TO_TOP'])) ? $this->_rootref['L_BACK_TO_TOP'] : ((isset($user->lang['BACK_TO_TOP'])) ? $user->lang['BACK_TO_TOP'] : '{ BACK_TO_TOP }')); ?>"><?php echo ((isset($this->_rootref['L_BACK_TO_TOP'])) ? $this->_rootref['L_BACK_TO_TOP'] : ((isset($user->lang['BACK_TO_TOP'])) ? $user->lang['BACK_TO_TOP'] : '{ BACK_TO_TOP }')); ?></a></div>
				</div>
			<?php } ?>
