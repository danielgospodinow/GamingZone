<?php if (!defined('IN_PHPBB')) exit; $_forumrow_count = (isset($this->_tpldata['forumrow'])) ? sizeof($this->_tpldata['forumrow']) : 0;if ($_forumrow_count) {for ($_forumrow_i = 0; $_forumrow_i < $_forumrow_count; ++$_forumrow_i){$_forumrow_val = &$this->_tpldata['forumrow'][$_forumrow_i]; if (( $_forumrow_val['S_IS_CAT'] && ! $_forumrow_val['S_FIRST_ROW'] ) || $_forumrow_val['S_NO_CAT']) {  ?>
	...
	<?php } if ($_forumrow_val['S_IS_CAT'] || $_forumrow_val['S_FIRST_ROW'] || $_forumrow_val['S_NO_CAT']) {  ?>
		...
		<div class="forums-wrapper">
			<table class="forums">
				<thead>
					<tr>
						<th colspan="3"><?php if ($_forumrow_val['S_IS_CAT']) {  ?><a href="<?php echo $_forumrow_val['U_VIEWFORUM']; ?>"><?php echo $_forumrow_val['FORUM_NAME']; ?></a><?php } else { echo ((isset($this->_rootref['L_FORUM'])) ? $this->_rootref['L_FORUM'] : ((isset($user->lang['FORUM'])) ? $user->lang['FORUM'] : '{ FORUM }')); } ?></th>
	...
	<?php } if (! $_forumrow_val['S_IS_CAT']) {  ?>
		...
		<tr class="row <?php if (($_forumrow_val['S_ROW_COUNT'] & 1)  ) {  ?>bg1<?php } else { ?>bg2<?php } ?>">
			<td class="icon bg2" style="background-image: url(<?php echo $_forumrow_val['FORUM_FOLDER_IMG_SRC']; ?>);" title="<?php echo $_forumrow_val['FORUM_FOLDER_IMG_ALT']; ?>"></td>
			<?php if ($_forumrow_val['FORUM_IMAGE']) {  ?><td class="forum-image"><?php echo $_forumrow_val['FORUM_IMAGE']; ?></td><?php } ?>

			<td class="forum" <?php if (! $_forumrow_val['FORUM_IMAGE']) {  ?>colspan="2"<?php } ?>>
			...
			<?php } else if (! $_forumrow_val['S_IS_LINK']) {  ?>
				...
				<td class="forum">
					<?php if ($_forumrow_val['LAST_POST_TIME']) {  if (! $this->_rootref['S_IS_BOT']) {  ?><a href="<?php echo $_forumrow_val['U_LAST_POST']; ?>" class="last-post-img"><?php echo (isset($this->_rootref['LAST_POST_IMG'])) ? $this->_rootref['LAST_POST_IMG'] : ''; ?></a><?php } ?>

						<div class="lastpost-details">
							<p><?php echo $_forumrow_val['LAST_POST_TIME']; ?></p>
							<p><?php echo ((isset($this->_rootref['L_POST_BY_AUTHOR'])) ? $this->_rootref['L_POST_BY_AUTHOR'] : ((isset($user->lang['POST_BY_AUTHOR'])) ? $user->lang['POST_BY_AUTHOR'] : '{ POST_BY_AUTHOR }')); ?> <?php echo $_forumrow_val['LAST_POSTER_FULL']; ?></p>
						</div>
					<?php } else { ?>

						<p style="text-align: center;"><?php echo ((isset($this->_rootref['L_NO_POSTS'])) ? $this->_rootref['L_NO_POSTS'] : ((isset($user->lang['NO_POSTS'])) ? $user->lang['NO_POSTS'] : '{ NO_POSTS }')); ?></p>
					<?php } ?>
