...
<?php } if ($this->_rootref['S_SHOW_GROUP']) {  ?>
		...
		<div class="panel">
			...
			<ul class="linklist">
				<li>
					...
					<?php $_first_char_count = (isset($this->_tpldata['first_char'])) ? sizeof($this->_tpldata['first_char']) : 0;if ($_first_char_count) {for ($_first_char_i = 0; $_first_char_i < $_first_char_count; ++$_first_char_i){$_first_char_val = &$this->_tpldata['first_char'][$_first_char_i]; ?>

						<a href="<?php echo $_first_char_val['U_SORT']; ?>"><?php echo $_first_char_val['DESC']; ?></a>&nbsp; 
					<?php }} ?>
	...
	<?php } $_memberrow_count = (isset($this->_tpldata['memberrow'])) ? sizeof($this->_tpldata['memberrow']) : 0;if ($_memberrow_count) {for ($_memberrow_i = 0; $_memberrow_i < $_memberrow_count; ++$_memberrow_i){$_memberrow_val = &$this->_tpldata['memberrow'][$_memberrow_i]; if ($this->_rootref['S_SHOW_GROUP']) {  if (! $_memberrow_val['S_GROUP_LEADER'] && ! $this->_tpldata['DEFINE']['.']['S_MEMBER_HEADER']) {  if ($this->_rootref['S_LEADERS_SET']) {  ?>
		...
		<tr class="<?php if (($_memberrow_val['S_ROW_COUNT'] & 1)  ) {  ?>bg2<?php } else { ?>bg1<?php } ?>">
			<td>
				...
				<?php if (( $this->_rootref['S_IN_SEARCH_POPUP'] && ! $this->_rootref['S_SELECT_SINGLE'] ) || $this->_rootref['S_SELECT_SINGLE']) {  ?>
	...
	<?php }} else { ?>
		...
		<tr class="bg1">
			<td colspan="<?php if ($this->_rootref['S_VIEWONLINE']) {  ?>6<?php } else { ?>5<?php } ?>"><p><?php echo ((isset($this->_rootref['L_NO_MEMBERS'])) ? $this->_rootref['L_NO_MEMBERS'] : ((isset($user->lang['NO_MEMBERS'])) ? $user->lang['NO_MEMBERS'] : '{ NO_MEMBERS }')); ?></p></td>
		</tr>
	<?php } ?>
